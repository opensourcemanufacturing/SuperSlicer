    ...
    */
    ...
    if(FLAVOR_IS(gcfopenfl)){ 
        ...
        if (m_last_z > 0.000){ // If this is not the first layer do this:
            m_z_move = (m_pos.z()) - (m_last_z) * 400.000; // layer height = next z move minus last z move
            ...
            gcode << "\n";
            gcode << "0x03 ZMove ";
            gcode << int(XYZF_NUM(m_z_move)) - 2000; // unpeel and set for next layer
            gcode << "\n";
            return gcode.str();
            ...
            gcode << "\n";
            gcode << "0x03 ZMove ";
            gcode << int(XYZF_NUM(m_z_move); // first layer height in microsteps
            gcode << "\n";
            return gcode.str();
        }
