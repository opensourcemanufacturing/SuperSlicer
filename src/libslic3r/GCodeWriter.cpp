    ...
    if(FLAVOR_IS(gcfopenfl)){ 
        ...
        m_pos.z() = z; // value of next Z move in mm
         ...
         if (m_last_z > 0.){ // If this is not the first layer do this:
             ...
             gcode << "\n";
             gcode << "0x03 ZMove ";
             gcode << "\nThis stuff ->"; gcode << m_last_z = m_pos.z(); gcode << " , "; gcode << m_pos.z() = z; gcode << " , "; gcode << int(m_z_move - microsteps_5mm); gcode << "\n";
             ...
             gcode << "\n";
             gcode << "This stuff ->"; gcode << m_last_z = m_pos.z(); gcode << " , "; gcode << m_pos.z() = z; gcode << " , "; gcode << int(m_z_move); gcode << "\n";
             return gcode.str();
         }
